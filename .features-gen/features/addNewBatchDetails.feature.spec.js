...
test.describe('Add New Batch details pop up', () => {
  ...
  test('Validate close icon on the batch details pop up', { tag: ['@batch'] }, async ({ Given, When, Then }) => { 
    await Given('Admin is on the Batch Details Pop Up Window'); 
    await When('Admin clicks on the close icon'); 
    await Then('batch details pop up closes'); 
  });
